...
# Fully functional toy dataset
    ...
    object %<>% filter_samples(cohort == 'COVID-19')
    complete_subjects <- data.table(sdata(object))[, 
        .SD[all(c('early', 'late') %in% time_since_onset)], 
        by='subject_id']$subject_id
    object %<>% filter_samples(subject_id %in% complete_subjects)
    object %<>% preprocess_rnaseq_counts(subgroupvar = 'subject_id')
# NULL case
    fit_limma(subtract_controls(
        object, block='subject_id', subgroupvar = 'time_since_onset'))
    object$subgroup <- NULL
    object %<>% fit_limma()                                 # NULL
    object$subgroup <- object$time_since_onset
    object %<>% fit_limma()                                 # 'subgroup'
    object %<>% fit_limma(subgroupvar = 'time_since_onset') # subgroupvar
    object %<>% fit_limma(formula = ~ 0 + time_since_onset) # formula
    object %<>% filter_features(F.p.limma < 0.05, verbose=TRUE)
    object %<>% fit_limma(subgroupvar = 'time_since_onset', block='subject_id')
    
    
    
    msg <- 'fit: unpaired'
    object %<>% fit_limma()
    object %<>% fit_lm(object)
