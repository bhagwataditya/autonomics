...
object <-  read_rnaseq_counts(file = file, fit = 'limma', plot = TRUE, label = 'gene_name')
```
...
# log2counts
  object <- read_rnaseq_counts(file, 
       cpm = FALSE, voom = FALSE, fit = 'limma', verbose = FALSE)
  colSums(summarize_fit(fdt(object), 'limma')[-1, c(3,4)])
# log2cpm
  object <- read_rnaseq_counts(file,
       cpm = TRUE,  voom = FALSE, fit = 'limma', verbose = FALSE)
  colSums(summarize_fit(fdt(object), 'limma')[-1, c(3,4)])
# log2cpm + voom
  object <- read_rnaseq_counts(file,  # log2 cpm + voom
       cpm = TRUE,  voom = TRUE,  fit = 'limma', verbose = FALSE)
  colSums(summarize_fit(fdt(object), 'limma')[-1, c(3,4)])
```
...
object <- read_maxquant_proteingroups(file = file, fit = 'limma')
```
...
object <- read_metabolon(file = file, subgroupvar = 'SET', fit = 'limma', block = 'SUB', plot = TRUE)
```
...
object <- read_somascan(file = file, fit = 'limma', block = 'Subject_ID', plot = TRUE)
```
