...
Autonomics offers read functions for several omics platforms, which import platform-specific data into a common SummarizedExperiment format:

```{r, echo = FALSE, out.width = '80%', out.height = '80%', message = FALSE}
require(data.table)
require(ggplot2)
require(autonomics)
dt <- data.table(
    x = 0:7, xend = 0:7, y = 2, yend = 1, 
    label = c('', 'read_affy', 'read_proteingroups', 'read_phosphosites', 
              'read_diann', 'read_somascan', 'read_metabolon', 
              'read_rnaseq_counts'), 
...
object.rnaseq        <- read_rnaseq_counts(file.rnaseq,        plot = FALSE)
object.proteingroups <- read_proteingroups(file.proteingroups, plot = FALSE)
object.phosphosites  <- read_phosphosites( file.phosphosites,
                                           file.proteingroups, plot = FALSE)
object.diann         <- read_diann(        file.diann,         plot = FALSE)
object.somascan      <- read_somascan(     file.somascan,      plot = FALSE)
object.metabolon     <- read_metabolon(    file.metabolon,     plot = FALSE)
```
