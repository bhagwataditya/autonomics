...
test_that("read_maxquant_proteingroups(file, invert_subgroups)", {
    file <- download_data('billing16.proteingroups.txt')
...
if (requireNamespace('Biostrings', quietly = TRUE)){
    test_that("read_maxquant_proteingroups(file, fastafile)", {
        ...
        object0 <- read_maxquant_proteingroups(file, invert = invert, plot=FALSE)
        object  <- read_maxquant_proteingroups(file, invert = invert, fastadt = fastadt, plot = FALSE)
        expect_s4_class(object, 'SummarizedExperiment')
...
test_that(  "read_maxquant_proteingroups(file, pca=TRUE)", {
    ...
    object <- read_maxquant_proteingroups(file, invert_subgroups = invert_subgroups, 
                                pca = TRUE, plot = FALSE)
    expect_s4_class(object, 'SummarizedExperiment')
