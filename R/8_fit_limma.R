...
prep_model_summary <- function(object, fit = fits(object)[1], nmax = NULL){
# Assert
    color <- coef1 <- coef2 <- y <- n <- NULL
# Prepare
     ups <- mapply(   upfeatures, coef = coefs(object), MoreArgs = list(object = object))
     dns <- mapply( downfeatures, coef = coefs(object), MoreArgs = list(object = object))
     nup <- matrix(0, nrow = length(ups), ncol = length(ups), dimnames = list(names(ups), names(ups)) )
     ndn <- matrix(0, nrow = length(dns), ncol = length(dns), dimnames = list(names(dns), names(dns)) )
     
    for (contrast1 in names(ups)){
    for (contrast2 in names(ups)){
        nup[contrast1, contrast2] <- length(intersect( ups[[contrast1]], ups[[contrast2]]))
        ndn[contrast1, contrast2] <- length(intersect( dns[[contrast1]], dns[[contrast2]]))
    }}

    updt <- mat2dt(nup, idvar = 'coef1')
    dndt <- mat2dt(ndn, idvar = 'coef1')
    updt %<>% melt.data.table(id.vars = 'coef1', variable.name = 'coef2', value.name = 'n')
    dndt %<>% melt.data.table(id.vars = 'coef1', variable.name = 'coef2', value.name = 'n')
    updt[, coef1 := factor(coef1, unique(coef1))];  dndt[, coef1 := factor(coef1, unique(coef1))]
    updt[, coef2 := factor(coef2, unique(coef2))];  dndt[, coef2 := factor(coef2, unique(coef2))]
    dndt[, x := as.numeric(coef1)]; dndt[, y := as.numeric(coef2)]
    updt[, x := as.numeric(coef1)]; updt[, y := as.numeric(coef2)]
...
#' object <- read_metabolon(file)
#' par(mfrow = c(2,4))
#' plot_model_summary(fit_wilcoxon(object, block = 'Subject'), title = 'wilcoxon', ylab = 'subgroup | block', nmax = 10)
#' plot_model_summary(      fit_lm(object, block = 'Subject'), title = 'lm',       ylab = ' ', nmax = 10)
#' plot_model_summary(   fit_limma(object, block = 'Subject'), title = 'limma',    ylab = ' ', nmax = 10)
#' plot_model_summary(     fit_lme(object, block = 'Subject'), title = 'lme',      ylab = ' ', nmax = 10)
#' plot_model_summary(fit_wilcoxon(object),                    title = ' ',        ylab = 'subgroup', nmax = 10)
#' plot_model_summary(      fit_lm(object),                    title = ' ',        ylab = ' ', nmax = 10)
#' plot_model_summary(   fit_limma(object),                    title = ' ',        ylab = ' ', nmax = 10)
#' plot(0, type = 'n', axes = FALSE, ann = FALSE)
#' @export
plot_model_summary <- function(object, fit = fits(object)[1], title = fit, sub = '', xlab = '', ylab = '', nmax = NULL){
# Assert
    assert_is_valid_sumexp(object)
    ...
    assert_is_a_number(nmax)
    x <- y <- color <- n <- coef1 <- coef2 <- NULL
# Prepare
    plotdt <- prep_model_summary(object, fit = fit, nmax = nmax)
    dd <- 0.30
    k <- length(levels(plotdt$coef1))
# Plot
    plot(1, type = 'n', xlab = '', ylab = '', xlim = c(-0.3,k), ylim = c(-k,0.3), axes = 0)  # axes = 1
    subdt <- plotdt[x==y & dir=='up'    ]; for (j in 1:nrow(subdt)) subdt[j][ , polygon(x = c(x-1,x,x    ), y = -c(y-1,y-1,y  ), col = color)]
    subdt <- plotdt[x==y & dir=='down'  ]; for (j in 1:nrow(subdt)) subdt[j][ , polygon(x = c(x-1,x,x-1  ), y = -c(y-1,y,  y  ), col = color)]
    subdt <- plotdt[x!=y];                 for (j in 1:nrow(subdt)) subdt[j][ , polygon(x = c(x-1,x,x,x-1), y = -c(y-1,y-1,y,y), col = color)]
    ...
    subdt <- plotdt[y==1 & dir == 'up'  ]; for (j in 1:nrow(subdt)) subdt[ , text(x = x-0.5, y = 0.2,    label = coef1) ]
    subdt <- plotdt[x==1 & dir == 'down']; for (j in 1:nrow(subdt)) subdt[ , text(x = -0.2,  y = -y+0.5, label = coef2)]
    plotdt[, polygon(x = c(0,max(x), max(x)), y = c(0,       0, -max(y)), border = 'black', lwd = 2)]
    plotdt[, polygon(x = c(0,max(x), 0     ), y = c(0, -max(y), -max(y)), border = 'black', lwd = 2)]
    title(main = title, sub = sub, line = 1, xlab = xlab, ylab = ylab)
}
