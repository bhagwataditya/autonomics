...
file.diann           <- download_data('dilution.report.tsv'     )
file.somascan        <- download_data('atkin18.somascan.adat'      )
file.metabolon       <- download_data('atkin18.metabolon.xlsx'     )

object.rnaseq        <- read_rnaseq_counts(         file = file.rnaseq)
object.proteingroups <- read_maxquant_proteingroups(file = file.proteingroups)
object.phosphosites  <- read_maxquant_phosphosites( phosphofile = file.phosphosites, proteinfile = file.proteingroups)
object.diann         <- read_diann_proteingroups(   file = file.diann)
object.somascan      <- read_somascan(              file = file.somascan)
object.metabolon     <- read_metabolon(             file = file.metabolon)
```
...
In this case **`r n[[4]]` proteingroups** (out of `r n[[1]]` identified) were retained for  analysis, after applying the following filters:
