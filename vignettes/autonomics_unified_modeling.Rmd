...
object <- read_metabolon(file)
object %<>% fit_lm(   ~ SET)
object %<>% fit_limma(~ SET) # prefers large effects over small sds
plot_contrast_venn(is_sig(object, fit = fits(object), contrast = 't3'))
...
```{r}
object <- read_metabolon(file)
object %<>% fit_limma(~ SET, block = 'SUB')
object %<>% fit_lme(  ~ SET, block = 'SUB')
object %<>% fit_lmer( ~ SET, block = 'SUB') # results identical for this basic 
plot_contrast_venn(is_sig(object, fit = fits(object), contrast = 't3'))
# plot_contrast_boxplots(object, subgroupvar='SET', contrast='t3', fit = c('lm', 'limma'))
```
...
```{r}
object <- read_metabolon(file)
object %<>% fit_limma(~0+SET, contrasts = c('t1-t0', 't2-t0', 't3-t0'), block = 'SUB')
object %<>% fit_limma(~0+SET, contrasts = c('t1-t0', 't2-t1', 't3-t2'), block = 'SUB')
```
...
```{r}
object <- read_metabolon(file)
object %<>% fit_limma(~SET, block = 'SUB')  # baseline diffs
object %<>% fit_limma(~SET, block = 'SUB', codingfun = code_diff)  # backward diffs
```
...
```{r}
object <- read_metabolon(file)
object %<>% fit_limma(   ~0+SET, contrasts = c('t3-t0'), block = 'SUB')
object %<>% fit_wilcoxon(~0+SET, contrasts = c('t3-t0'), block = 'SUB')
plot_contrast_venn(is_sig(object, contrast = 't3-t0', fit = fits(object)))
```
