...
By default R uses treatment coding, which means the $\beta_{\textrm{Intercept}}$ represents the level of the first subgroup, and subsequent coefficients (e.g. $\beta_{\textrm{Adult}}$) differences to that first subgroup:

```{r, fig.width = 5, fig.height = 3, out.width='30%', out.height='30%'}
plot_design(object)
```
...
```{r, fig.width = 5, fig.height = 3, out.width='30%', out.height='30%', message = FALSE}
...
In default treatment coding, the coefficients express differences to the reference subgroup.
It might, however, be more interesting to quantify successive subgroup differences.
