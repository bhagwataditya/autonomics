...
```{r, fig.width = 5, fig.height = 3, out.width='50%', out.height='50%'}
plot_design(object)
```

This can be changed, and to understand why this can be useful let's use Billing et al. (2019)'s stem cell differentiation, in which Embryonic Stem cells (subgroup E00) are differentiated into Mesenchymal Stem Cells (M00), over a period of multiple days (E01, E02, E05, E15, E30). Default treatment coding fits a model like discussed above: 

```{r, fig.width = 5, fig.height = 3, out.width='50%', out.height='50%', message = FALSE}
...
In default treatment coding, the coefficients express baseline differences.
It might, however, be interesting to quantify successive rather than baseline differences.
This can be done using the `MASS::contr.sdif` (successive differences) coding scheme:
...
object %<>% fit_limma(formula = ~ 0 + subgroup, contrastdefs = c('Adult - X30dpt'))  # rely on defaults
```
